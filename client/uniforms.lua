...
local function refresh()
  local list, canManage, faction = lib.callback.await(
    "st_appearance:server:getFactionUniforms", false
  )
...
local function promptSave(existing)
  ...
  TriggerServerEvent("st_appearance:server:saveFactionUniform", uniform)
...
local function confirmDelete(uniform)
  ...
  TriggerServerEvent("st_appearance:server:deleteFactionUniform", uniform.id)
  SetTimeout(400, function()
    refresh()
    uniforms.open()
  end)
...
local function buildManageMenu(uniform)
  lib.registerContext({
    id = "st_appearance_uniform_manage_" .. uniform.id,
    title = uniform.name,
    menu = "st_appearance_uniforms",
    options = {
      {
...
function uniforms.apply(uniform)
  ...
  TriggerServerEvent("st_appearance:server:saveAppearance", fullAppearance)
end
...
function uniforms.open()
  ...
  for _, uniform in ipairs(cached) do
    ...
    if cachedCanManage then
      entry.menu = "st_appearance_uniform_manage_" .. uniform.id
    end
  ...
  lib.showContext("st_appearance_uniforms")
end
