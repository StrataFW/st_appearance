...
function admin.saveForPlayer(appearance)
  ...
  TriggerServerEvent("st_appearance:server:adminSaveAppearance", admin.targetSource, appearance)
...
function admin.close()
  ...
  local ourAppearance <const> = lib.callback.await("st_appearance:server:getAppearance", false)
...
function admin.init()
  ...
  RegisterCommand(config.admin.command or "setappearance", function(_, args)
    ...
    TriggerServerEvent("st_appearance:server:adminRequestAppearance", targetId)
  end, false)
