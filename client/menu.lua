...
local defaultTabs <const> = {
  ...
  "presets", "outfits", "wardrobe", "camera", "history", "animations",
}
...
function menu.open()
  ...
  local userPresets <const> = lib.callback.await("st_appearance:server:getPresets", false)
  if userPresets then
...
function menu.close(save)
  ...
  TriggerEvent('st_appearance:closed', save == true)
end
